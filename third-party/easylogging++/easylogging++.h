...
//If GCC, errors on MSVC.
#ifdef __GNUC__
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wnull-dereference"
#endif
    // Causes an error on GCC.
...
#if ELPP_VARIADIC_TEMPLATES_SUPPORTED
template <typename T, typename... Args>
void Logger::log_(Level varlevel, int vlevel, const char* s, const T& value, const Args&... args) {
...
template <typename T>
void Logger::log_(Level varlevel, int vlevel, const T& vlog) {
  if (varlevel == Level::Verbose) {
    if (ELPP->vRegistry()->allowed(static_cast<base::type::VerboseLevel>(vlevel), __FILE__)) {
      base::Writer(Level::Verbose, "FILE", 0, "FUNCTION",
                   base::DispatchAction::NormalLog, vlevel).construct(this, false) << vlog;
    } else {
      stream().str(ELPP_LITERAL(""));
      releaseLock();
    }
  } else {
    base::Writer(varlevel, "FILE", 0, "FUNCTION").construct(this, false) << vlog;
  }
}
template <typename T, typename... Args>
inline void Logger::log(Level varlevel, const char* s, const T& value, const Args&... args) {
  acquireLock(); // released in Writer!
  log_(varlevel, 0, s, value, args...);
}
template <typename T>
inline void Logger::log(Level varlevel, const T& vlog) {
  acquireLock(); // released in Writer!
  log_(varlevel, 0, vlog);
}
#  if ELPP_VERBOSE_LOG
template <typename T, typename... Args>
...
template <typename T>
inline void Logger::verbose(int vlevel, const T& vlog) {
  acquireLock(); // released in Writer!
  log_(el::Level::Verbose, vlevel, vlog);
}
#  else
template <typename T, typename... Args>
